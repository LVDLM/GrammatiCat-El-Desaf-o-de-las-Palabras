...
const TutorialSign: React.FC<{ 
  ...
  return (
    <div 
      className="fixed z-[150] w-[80%] max-w-[250px] transition-all duration-500 animate-in fade-in zoom-in cursor-pointer"
      style={{ top: coords.top, left: coords.left, transform: 'translateX(-50%)' }}
      onClick={onNext}
    >
      <div className={`relative p-3 rounded-2xl border-2 shadow-2xl ${isMidnight ? 'bg-slate-800 border-indigo-500 text-white' : 'bg-white border-yellow-400 text-indigo-900'}`}>
        <div className={`absolute w-0 h-0 border-l-[8px] border-l-transparent border-r-[8px] border-r-transparent left-1/2 -translate-x-1/2 ${coords.arrow === 'up' ? 'bottom-full border-b-[12px] ' + (isMidnight ? 'border-b-indigo-500' : 'border-b-yellow-400') : 'top-full border-t-[12px] ' + (isMidnight ? 'border-t-indigo-500' : 'border-t-yellow-400')}`}></div>
        <p className="text-xs font-black leading-tight mb-2 text-center">{text}</p>
        <div className="flex justify-center">
          <button className="px-4 py-1 bg-indigo-600 text-white text-[9px] font-black rounded-full uppercase">
            OK <i className="fas fa-check ml-1"></i>
          </button>
        </div>
      </div>
    </div>
...
const App: React.FC = () => {
  ...
  // Lógica de detección de orientación y pantalla completa automática
  useEffect(() => {
    const handleOrientationChange = () => {
      const isPortrait = window.innerHeight > window.innerWidth;
      // Si el usuario gira a vertical, salimos de pantalla completa automáticamente
  ...
  const checkAchievements = useCallback((updatedStats: GameState['stats']) => {
    ...
    const spanishDoneCount = validEsLevels.filter(l => updatedStats.completedLevels?.[l.id]?.length > 0).length;
    const univDoneCount = validUnivLevels.filter(l => updatedStats.completedLevels?.[l.id]?.length > 0).length;

    if (validEsLevels.length > 0 && spanishDoneCount >= validEsLevels.length / 2 && validUnivLevels.length > 0 && univDoneCount >= validUnivLevels.length / 2) unlockAchievement('half_world');
    if ((validEsLevels.length > 0 && spanishDoneCount === validEsLevels.length) || (validUnivLevels.length > 0 && univDoneCount === validUnivLevels.length)) unlockAchievement('max_knowledge');
    if (validEsLevels.length > 0 && spanishDoneCount === validEsLevels.length && validUnivLevels.length > 0 && univDoneCount === validUnivLevels.length) unlockAchievement('absolute_knowledge');
    ...
    if (currentLevel) {
      ...
      if (availableCats.length > 0 && availableCats.every(cat => completedCats.includes(cat))) {
    ...
    <div className={`min-h-[100dvh] w-full flex flex-col items-center justify-start py-4 md:py-8 p-2 md:p-4 transition-all duration-1000 ${showKonamiEffect ? 'bg-gradient-to-br from-slate-950 via-purple-950 to-indigo-950 konami-active' : 'bg-gradient-to-br from-blue-400 via-indigo-400 to-purple-500'} relative`}>
      {lastUnlocked && (
        <div className="fixed top-4 right-4 z-[100] bg-white p-3 rounded-xl shadow-2xl border-2 border-yellow-400 animate-in slide-in-from-right duration-500 flex items-center space-x-3">
          ...
          <div><span className="block text-[8px] font-black text-yellow-600 uppercase tracking-widest">¡Logro!</span><span className="block text-sm font-black text-indigo-900 whitespace-nowrap">{lastUnlocked.title}</span></div>
        </div>
      )}
      ...
      {view === GameView.PLAYING && currentLevel && gameState.targetCategory && (
        <div className="w-full h-full flex flex-col items-center p-1 md:p-2 transition-all duration-300 overflow-hidden relative">
          ...
          <div className="hud-container w-full max-w-6xl">
          ...
          <p className={`mt-1 text-[7px] md:text-[10px] font-black uppercase italic opacity-30 ${showKonamiEffect ? 'text-indigo-300' : 'text-indigo-900'}`}>{currentLevel.title}</p>
          ...
          {!gameState.isPlaying && !gameState.isGameOver && lastReward && (
            <div className="fixed inset-0 flex items-center justify-center z-[200] animate-in zoom-in duration-500 p-4">
              ...
              <div className="relative bg-white p-6 rounded-[2rem] shadow-2xl border-4 border-indigo-500 text-center max-w-sm w-full">
                 <h2 className="text-2xl md:text-4xl font-black text-indigo-900 mb-3 italic uppercase">¡MUY BIEN!</h2>
                 <div className="bg-indigo-50 p-4 rounded-2xl border-2 border-dashed border-indigo-200 mb-4 flex flex-col items-center">
                    <div className={`text-5xl md:text-7xl mb-2 animate-bounce ${lastReward.color}`}><i className={`fas ${lastReward.icon}`}></i></div>
                    <span className={`text-xl md:text-3xl font-black italic ${lastReward.color} uppercase`}>{lastReward.label}</span>
                 </div>
      ...
      {view === GameView.GAME_OVER && (
        <div className={`text-center p-6 md:p-10 rounded-[2rem] md:rounded-[3rem] shadow-2xl border-b-4 border-rose-500 animate-in zoom-in w-full max-sm mx-4 ${showKonamiEffect ? 'bg-slate-900' : 'bg-white'}`}>
          ...
          <div className="mb-6 p-4 bg-indigo-50 rounded-2xl border-2 border-indigo-100">
            <span className="text-4xl md:text-5xl font-black text-indigo-600 mb-1">{gameState.score}</span>
            <span className="text-[8px] font-bold text-slate-400 block uppercase tracking-widest">PUNTOS</span>
            {gameState.score > 0 && !scoreSaved && (
              <div className="mt-4 flex flex-col gap-2">
                <input type="text" value={playerName} onChange={e => setPlayerName(e.target.value)} placeholder="Tu nombre..." className="p-2 rounded-xl border-2 border-indigo-200 text-center font-bold text-sm" />
                <button onClick={async () => { setIsSavingScore(true); await saveScore({ name: playerName, score: gameState.score }); setScoreSaved(true); setIsSavingScore(false); }} disabled={!playerName || isSavingScore} className="py-2 bg-indigo-600 text-white rounded-xl font-black text-xs uppercase">GUARDAR PUNTOS</button>
              </div>
            )}
          </div>
          <button onClick={() => setView(GameView.MENU)} className="w-full py-3 bg-indigo-600 text-white rounded-xl text-sm md:text-base font-black uppercase italic">MENÚ PRINCIPAL</button>
        </div>
      )}
